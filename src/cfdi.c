...
int cfdi( char *folio ){
    ...
    sprintf(json, "{\"bd\":\"%s\",\"username\":\"%s\",\"password\":\"%s\",\"accion\":\"facturar\",\"folio\":\"%s\"}\0", bd, usuario, contrasena, folio);
...
int cfdi( char *desde, char *hasta ){
    ...
    sprintf(json, "{\"bd\":\"%s\",\"username\":\"%s\",\"password\":\"%s\",\"accion\":\"facturar_cierre\",\"desde\":\"%s\",\"hasta\":\"%s\"}\0", bd, usuario, contrasena, desde, hasta);
    printf("Cadena JSON a enviar al servidor: %s\n",json);
    write(sockfd, &json, strlen(json));
    while ( (n = read(sockfd, &respuesta, sizeof(respuesta)-1)) > 0)
    {
...
int cancelar_cfdi( char *id_factura ){
    int sockfd;
    int len, n = 0;
    struct sockaddr_in address;
    int result;
    ...
    sprintf(json, "{\"bd\":\"%s\",\"username\":\"%s\",\"password\":\"%s\",\"accion\":\"cancelar\",\"id_factura\":\"%s\"}\0", bd, usuario, contrasena, id_factura);
    printf("Cadena JSON a enviar al servidor: %s\n",json);
    write(sockfd, &json, strlen(json));
    while ( (n = read(sockfd, &respuesta, sizeof(respuesta)-1)) > 0)
    {
